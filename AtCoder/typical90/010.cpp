...
int main(){
    ...
    ll n, q;
    cin >> n;
    vector<pair<ll, ll>> class_point(n);
    for(auto& [l, r]:class_point) cin >> l >> r;
    cin >> q;
    vector<pair<ll, ll>> range(q);
    for(auto& [l, r]:range) cin >> l >> r;

    vector<ll> cum_sum1(n+1);
    vector<ll> cum_sum2(n+1);
    ll sum1 = 0;
    ll sum2 = 0;
    for(int i = 0; i < n; ++i){
        if(class_point[i].first == 1){
            sum1 += class_point[i].second;
        }else{
            sum2 += class_point[i].second;
        }
        cum_sum1[i+1] = sum1;
        cum_sum2[i+1] = sum2;
    }
    ...
    for(int i = 0; i < q; ++i){
        int ans1 = cum_sum1[range[i].second] - cum_sum1[range[i].first - 1];
        int ans2 = cum_sum2[range[i].second] - cum_sum2[range[i].first - 1];
        cout << ans1 << " " << ans2 << '\n';
    }
